...
export default {
  ...
  data() {
    ...
    const validateUsername = (rule, value, callback) => {
      if (value.length < 1) {
        callback(new Error("The password can not be less than 1 string"));
      } else if (value == " ") {
  ...
  methods: {
    ...
    AuthProvider(provider) {
      ...
      this.$auth
        ...
        .then(response => {
          ...
          self.SocialSignUp(provider, response);
        })
        .catch(err => {
          console.log({ err: err });
        });
    ...
    SocialSignUp(provider, response) {
      this.$store
        .dispatch("user/socialSignup", { provider, response })
        .then(() => {
          this.$router.push({
            path: this.redirect || "/",
            query: this.otherQuery
        ...
        .catch(() => {
          console.log("social signup failed");
        });
    }
  }
};
